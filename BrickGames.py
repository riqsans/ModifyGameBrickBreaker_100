...
class PowerUp(GameObject):
    def __init__(self, canvas, x, y, power_type):
        ...
        colors = {'expand': 'blue', 'slow': 'green', 'life': 'red'}
        item = canvas.create_oval(x - self.width / 2, y - self.height / 2,
                                  x + self.width / 2, y + self.height / 2,
                                  fill=colors[power_type], tags='powerup')
        super(PowerUp, self).__init__(canvas, item)

    def activate(self, game):
        if self.power_type == 'expand':
            game.paddle.width += 20
...
def check_collisions(self):
    ...
    for obj in objects:
        if isinstance(obj, Brick):
            ...
            self.score += 10
            self.update_score()
            # Kemungkinan menjatuhkan power-up
            if random.random() < 0.2:  # 20% peluang muncul power-up
                power_type = random.choice(['expand', 'slow', 'life'])
                powerup = PowerUp(self.canvas, *self.ball.get_position()[:2], power_type)
